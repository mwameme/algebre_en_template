...
les polynomes ont une multiplication, une addition, une soustraction, mais aussi une division (partielle) et son reste.
...
les rationnels ont les opérations + - * et / (exacte). Le reste (%) est mis à  (la division est exacte).

Quand on utilise les corps fini, attention que p soit premier, et le même des deux côtés (lors des opérations).
C'est un corps, donc il y a + - * /, et le reste est mis à 0 (utilisé lors du pgcd).
le "= bool test" met x = 0 ou 1

Idem, le corps des complexes est un corps ... vérifier avant division que le nombre est non-nul.

Toujours il y a la transformation d'un élément en booléen : si tous les coefficients sont nuls (polynome matrice corps-fini et complexe), ou si le numerateur est nul (fraction).
Au lieu d'avoir un contructeur qui prend un booléen en parametre, j'utilise la copie d'un élément que je transforme avec = true/false. Ca permet de garder les caractéristiques lorsqu'il y a plusieurs éléments neutres (pour les matrices il faut spécifier la taille aussi ... Voilà pourquoi (et aussi pour les corps finis)).

Au final toutes les opérations sont définies, avec la subtilité : la division est exacte (pour les corps), ou approchée (anneaux : polynomes ou entiers).
Il faut pouvoir mettre les éléments = true/false, et vérifier si un élément est nul (==? true/false)
Attention aux dépassements de la taille des entiers.
